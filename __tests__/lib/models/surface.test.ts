...
describe("boundingBox", () => {
  ...
  describe("a single rectangle centered on the origin", () => {
    ...
    test("the size of box be the size of the rectangle with margin", () => {
      expect(boundingBox([centeredRectangle], margin).width).toBe(
        width(centeredRectangle) + margin * 2
      );
      expect(boundingBox([centeredRectangle], margin).height).toBe(
        height(centeredRectangle) + margin * 2
      );
    });
  });
